...
texts = df["Yetenekler"].dropna().progress_apply(str)  
texts = texts.tolist()  
...
# django kelimesinin vektörünü alalım
django_index = feature_names.tolist().index('django')  # 'django' kelimesinin indeksini bul

# django kelimesinin TF-IDF vektörünü alıyoruz ve 2D formatta yapıyoruz
django_vector = tfidf_matrix[:, django_index].toarray()

# Tüm kelimelerin TF-IDF vektörlerini alıyoruz
tfidf_vectors = tfidf_matrix.toarray()

# Cosine similarity hesaplayalım
similarities = cosine_similarity(django_vector.T, tfidf_vectors.T)
